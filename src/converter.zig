        ...
        for (switch_type_mapping) |mapping| {
            ...
            for (switches) |sw| {
                ...
                if (!std.mem.eql(u8, voltage_level1_id, voltage_level2_id)) {
                    ...
                    try print.stdout("Error: conversion failed for {s} '{s}' because of a voltage level mismatch: '{s}' != '{s}'", .{ mapping.cim_type, name.?, voltage_level1.name.?, voltage_level2.name.? });
