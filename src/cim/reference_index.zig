...
pub const ReferenceIndex = struct {
    ...
    pub fn init(gpa: std.mem.Allocator, model: *const CimDocument) !ReferenceIndex {
        ...
        for (model.objects, 0..) |obj, index| {
            ...
            // Ambiguous aliases must fail lookup rather than select an arbitrary object.
    ...
    /// Already-local references were checked against raw IDs by the exact probe.
    ...
    /// Preserve ambiguity so the scope cannot accept another document's alias.
...
/// Both maps use StringContext, so the same hash serves every probe and document.
const HashedKey = struct {
    bytes: []const u8,
    /// A hash that differs from StringHashMap's silently misses existing keys.
    digest: u64,
...
pub const ReferenceScope = struct {
    ...
    /// Documents must outlive the scope because names and indexes borrow their storage.
    /// The document limit protects a fixed arbitration buffer even in ReleaseFast.
    pub fn init(gpa: std.mem.Allocator, cim_documents: []const *const CimDocument) !ReferenceScope {
        ...
        // The parser rejects duplicate IDs within a document, so only cross-document IDs contend.
    ...
    /// Arbitration rewrites type IDs, so the resolved element's own class may be stale.
    ...
    /// Probe each stage across all documents so a later exact match beats an earlier alias.
    ...
    /// Keep scanning after a hit: a later document can make the alias ambiguous.
        ...
        for (self.indexes, 0..) |*index, document_index| {
            switch (index.probe_unique_alias(local)) {
                ...
                .unique => |object_index| {
                    ...
                    // Repeated raw IDs are one identity; distinct raw IDs make the alias ambiguous.
    ...
    /// Settle types at build time to make lookups independent of document order.
    /// Quadratic work in the bounded document count avoids another allocation.
    fn arbitrate_collisions(
        ...
        for (indexes[1..], 1..) |*index, document_index| {
            ...
            for (index.model.objects) |object| {
                ...
                // The second occurrence settles all declarations, including later ones.
    ...
    /// Prefer the most specific class; unrelated classes use names for a stable tie-break.
    /// A running winner would discard candidates and make three-way conflicts order-dependent.
...
const LocalKey = struct {
    ...
    fn init(reference: []const u8, raw: HashedKey) LocalKey {
        ...
        // Local forms are suffixes, so equal lengths imply equal content.
...
test "ReferenceScope: three declarations settle the same under every permutation" {
    ...
    // This third class exposes order-dependent arbitration that folds pairwise winners.
    inline for (.{
...
test "ReferenceScope: init leaks nothing when any allocation fails" {
    ...
    // Allocate documents separately so only scope allocations are fault-injected.
...
test "ReferenceScope: arbitration reaches objects the alias rung resolves" {
    // Full IRIs force alias lookup; the shared raw ID must still use the arbitrated type.
    var a = try init_document(
        \\<rdf:RDF>
