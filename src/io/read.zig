...
pub fn read_path(io: std.Io, gpa: std.mem.Allocator, file_path: []const u8) ![]const u8 {
    if (is_stdin(file_path)) return read_stdin(io, gpa);
...
fn read_stdin(io: std.Io, gpa: std.mem.Allocator) ![]const u8 {
    ...
    const data = try stdin_reader.interface.allocRemaining(gpa, .limited(max_in_memory_input_bytes + 1));
    errdefer gpa.free(data);
    if (data.len > max_in_memory_input_bytes) return error.FileTooLarge;
