//! The closed rule inventory of QoCDC v4.1.4 as validated by this module:
//! 8 filename rules, 1 header rule, and 38 grid-model rules.
//!
//! Tags carry the official QoCDC rule names -- `@tagName` is the stable grep
//! key of the report line format (`qocdc: <Rule>: ...`), so renaming a tag is
//! a report-format change.
//!
//! Grid-model rules that read only the object under inspection are entries in
//! the comptime `object_rules` table: a type filter, a profile gate, the
//! child names the predicate reads (`needs`), and the predicate itself. The
//! engine (engine.zig) fuses every applicable entry into one child walk per
//! object and hands the predicate its values through `Slots`.
...
pub fn message(rule: Rule) []const u8 {
    return switch (rule) {
        ...
        .TerminalCount1 => "a single terminal equipment that is not referenced by exactly one terminal",
        .TerminalCount2 => "a two terminal equipment that is not referenced by exactly two terminals",
        .TerminalSeqNum => "a cim:Terminal of either a cim:EquivalentBranch or a cim:ACLineSegment " ++
        ...
        .MCFirstSecond => "cim:MutualCoupling.First_Terminal and cim:MutualCoupling.Second_Terminal " ++
