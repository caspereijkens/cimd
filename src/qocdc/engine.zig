...
//!
//! One walk over each object's children fills value slots for every table
//! entry active on its type group; predicates then run over the slots. Type
//! filters cost one `type_id` lookup plus bit tests per group (not per
